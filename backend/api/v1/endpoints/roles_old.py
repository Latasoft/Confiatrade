...
async def obtener_rol(
    ...
    Solo accesible por administradores
    """
    rol = db.query(RolModel).options(joinedload(RolModel.permisos)).filter(RolModel.id == rol_id).first()
...
async def crear_rol(
    ...
    rol_existente = db.query(RolModel).filter(RolModel.nombre == rol_data.nombre).first()
    if rol_existente:
        raise HTTPException(
    ...
    if rol_data.permisos_ids:
        permisos = db.query(PermisoModel).filter(PermisoModel.id.in_(rol_data.permisos_ids)).all()
...
async def eliminar_rol(
    ...
    usuarios_count = db.query(func.count(UsuarioModel.id)).filter(UsuarioModel.rol_id == rol_id).scalar()
    if usuarios_count > 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def asignar_permisos_a_rol(
    ...
    # Verificar que los permisos existan
    permisos = db.query(PermisoModel).filter(PermisoModel.id.in_(request.permisos_ids)).all()
...
async def listar_permisos(
    ...
    Solo accesible por administradores
    """
    logger.info(f"[PERMISOS] Iniciando listar_permisos - modulo={modulo}, activo={activo}")
    logger.info(f"[PERMISOS] Usuario actual: {current_user.email}, rol={current_user.rol}, rol_id={current_user.rol_id}")
...
async def listar_permisos_por_modulo(
    ...
    Solo accesible por administradores
    """
    permisos = db.query(PermisoModel).filter(PermisoModel.activo).order_by(PermisoModel.modulo, PermisoModel.nombre).all()
...
async def crear_permiso(
    ...
    permiso_existente = db.query(PermisoModel).filter(PermisoModel.nombre == permiso_data.nombre).first()
