...
class RealizarCheckInUseCase:
    ...
    def execute(
        ...
        """
        print("[USE CASE] Ejecutando check-in")
        ...
        if participante.check_in_realizado and not force:
            print("  ✗ Ya tiene check-in y force=False → RECHAZADO")
        ...
        if participante.check_in_realizado and force:
            print("  ⚠ Ya tiene check-in pero force=True → PERMITIENDO")
        ...
        if qr_data_json and not force:
            ...
            if qr_tipo != "participante":
                print(
                    f"  ✗ QR tipo incorrecto: esperado 'participante', recibido '{qr_tipo}'"
                )
                raise ValidationException(
            ...
            if qr_id != str(participante_id):
                print(
                    f"  ✗ QR ID no coincide: esperado '{participante_id}', recibido '{qr_id}'"
                )
                raise ValidationException(
                    message="El código QR no corresponde a este participante"
                )

            print("  ✓ QR válido y coincide con el participante")
        elif qr_data_json and force:
            print("  ⚠ QR ignorado por force=True")
        else:
            print("  → Sin QR, realizando check-in directo")

        # 4. Actualizar participante directamente (ya lo tenemos cargado con empresa)
        print("  → Actualizando check-in en base de datos...")
